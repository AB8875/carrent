...
import { myActivityData } from "@/components/helper/user/Helper";
import { LeftArrowIcon } from "@/components/helper/user/Icon";

function SavedTabData() {
  const pathname = usePathname();

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 8;
  const totalPages = Math.ceil(myActivityData.length / itemsPerPage);
  ...
  const paginatedData = myActivityData.slice(startIndex, endIndex);
  return (
    <div>
      <div
