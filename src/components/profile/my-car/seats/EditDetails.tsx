    ...
    <div>
      ...
      <div className="mx-auto max-w-[1720px] px-4 pt-10 sm:px-6 md:px-8 lg:px-10 lg:pt-12 xl:px-12 xl:pt-[77px]">
          ...
          >
            <TabsList className="mb-8 flex w-full flex-wrap justify-center sm:mb-14 md:mb-16 lg:mb-18 lg:flex-nowrap">
              {tabName.map((item, index) => (
                <TabsTrigger
                  key={index}
                  className={`data-[state=active]:text-primary text-gray flex w-full border-0 text-xl leading-[144%] font-semibold !shadow-none md:text-2xl lg:text-[28px] ${index === 0 ? "!text-primary" : index === 1 ? "!text-primary" : index === 2 ? "!text-primary" : ""} ${index === 3 ? "!text-gray" : ""}`}
                  value={item.value}
                >
        ...
        <div className="mt-8 flex flex-col gap-3 md:mt-10 md:flex-row md:gap-5">
          <CommonYellowBtn title="Save Changes" link="#" className="md:w-max" />
